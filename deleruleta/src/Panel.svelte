<script lang="ts">
    ...
    const num_filas = 4;
    const num_cols = 16;
    const num_chars = 14*2 + num_cols*(num_filas-2); 
    ...
    function check_letter(input: HTMLElement | null) {
        if (input == null) {
            return;
        }
        let inputHTML = input as HTMLInputElement;
        ...
        let letra = inputHTML.value.toUpperCase();
        if (letra.length > 1){
    ...
    // Manejo de clicks
    function clicking_cell(position: string) {
        panel[position].clickable = false;
        panel[position].visible = true;
        panel[position].color = "bg-[#FFFFFF]";
    }
